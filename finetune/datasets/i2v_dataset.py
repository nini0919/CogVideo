...
class DPOI2VDataset(Dataset):
    ...
    def __init__(
        ...
        data_root: str,
        caption_column: str,
        win_video_column: str,
        lose_video_column: str,
        image_column: str | None,
        device: torch.device,
        trainer: "Trainer" = None,
        *args,
        **kwargs,
        ...
        data_root = Path(data_root)
        reject_data_root = Path("reject_sampling/hug_debug")
        self.prompts = load_prompts(data_root / caption_column)
        self.win_videos = load_videos(data_root / win_video_column)
        self.lose_videos = load_videos(reject_data_root/ lose_video_column)
        if image_column is not None:
            self.images = load_images(data_root / image_column)
        else:
            self.images = load_images_from_videos(self.win_videos)
        self.trainer = trainer
        ...
        # Check if number of prompts matches number of videos and images
        if not (len(self.win_videos) == len(self.prompts) == len(self.images)):
            raise ValueError(
                f"Expected length of prompts, videos and images to be the same but found {len(self.prompts)=}, {len(self.videos)=} and {len(self.images)=}. Please ensure that the number of caption prompts, videos and images match in your dataset."
            )

        # Check if all video files exist
        if any(not path.is_file() for path in self.win_videos):
            raise ValueError(
                f"Some video files were not found. Please ensure that all video files exist in the dataset directory. Missing file: {next(path for path in self.videos if not path.is_file())}"
            )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        prompt = self.prompts[index]
        video = self.win_videos[index]
        lose_video = self.lose_videos[index]
        image = self.images[index]
        train_resolution_str = "x".join(str(x) for x in self.trainer.args.train_resolution)

        cache_dir = self.trainer.args.data_root / "cache"
        video_latent_dir = cache_dir / "video_latent" / self.trainer.args.model_name / train_resolution_str
        lose_video_latent_dir = cache_dir / "lose_video_latent" / self.trainer.args.model_name / train_resolution_str

        prompt_embeddings_dir = cache_dir / "prompt_embeddings"
        video_latent_dir.mkdir(parents=True, exist_ok=True)
        prompt_embeddings_dir.mkdir(parents=True, exist_ok=True)
        lose_video_latent_dir.mkdir(parents=True, exist_ok=True)
        prompt_hash = str(hashlib.sha256(prompt.encode()).hexdigest())
        prompt_embedding_path = prompt_embeddings_dir / (prompt_hash + ".safetensors")
        encoded_video_path = video_latent_dir / (video.stem + ".safetensors")
        ...
        # load lose video
        lose_id = random.randint(0,self.sample_num-1)
        encoded_lose_video_path =  lose_video_latent_dir / (video.stem / Path(f"{lose_id}.safetensors"))
        if not (lose_video_latent_dir / video.stem).exists():
            (lose_video_latent_dir / video.stem).mkdir(parents=True, exist_ok=True)

        if encoded_lose_video_path.exists():
            encoded_lose_video = load_file(encoded_lose_video_path)["encoded_video"]
        else:
            # save_lose_video
            for k in range(self.sample_num):
                frames, _ = self.preprocess(lose_video/ f"{k}.mp4",None)
                frames = frames.to(self.device)
                frames = frames.unsqueeze(0)
                frames = frames.permute(0, 2, 1, 3, 4).contiguous()
                encoded_lose_video = self.encode_video(frames)
                encoded_lose_video_path =  lose_video_latent_dir / (video.stem / Path(f"{k}.safetensors"))
                encoded_lose_video = encoded_lose_video[0]
                encoded_lose_video = encoded_lose_video.to("cpu")
                save_file({"encoded_video": encoded_lose_video}, encoded_lose_video_path)
                logger.info(f"Saved encoded video to {encoded_lose_video_path}", main_process_only=False)
        ...
        return {
            "image": image,
            "prompt_embedding": prompt_embedding,
            "encoded_win_video": encoded_video,
            "encoded_lose_video": encoded_lose_video,
            "video_metadata": {
                "num_frames": encoded_video.shape[1],
                "height": encoded_video.shape[2],
                "width": encoded_video.shape[3],
...
class DPOI2VDatasetWithResize(DPOI2VDataset):
    """
    A dataset class for image-to-video generation that resizes inputs to fixed dimensions.
